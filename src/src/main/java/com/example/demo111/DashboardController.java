...
public class DashboardController implements Initializable {
    ...
    @FXML
    private TableView<CashPayment> cashpaymenttable;

    @FXML
    private TableColumn<CashPayment,String> cashpaymentcol;

    @FXML
    private TableColumn<FullBenefit, String> fullbenefitcol;

    @FXML
    private TableColumn<FullBenefit, String> fullbenefitidcol;

    @FXML
    private TableView<FullBenefit> fullbenefittable;

    @FXML
    private TableColumn<HalfBenefit, String> halfbenefitcol;

    @FXML
    private TableColumn<HalfBenefit, String> halfbenefitidcol;

    @FXML
    private TableView<HalfBenefit> halfbenefittable;


    @FXML
    private TableColumn<NoBenefit, String> nobenefitcol;

    @FXML
    private TableColumn<NoBenefit, String> nobenefitidcol;

    @FXML
    private TableView<NoBenefit> nobenefittable;
    @FXML
    ...
    private ObservableList<CashPayment> cashPaymentList = addCashPayment();

    private ObservableList<FullBenefit> fullBenefitList = addFullBenefit();

    private ObservableList<HalfBenefit> halfbenefitList = addHalfBenefit();

    private ObservableList<NoBenefit> noBenefitsList = addNoBenefit();
    ...
    public ObservableList<CashPayment> addCashPayment() {
        ObservableList<CashPayment> cashPaymentList = FXCollections.observableArrayList();
        String sqlQuery = "SELECT * FROM CASH_PAYMENT";
        try {
            connection = DatabaseController.connection();
            preparedStatement = connection.prepareStatement(sqlQuery);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                int cashPaymentId = resultSet.getInt("reciept_num");
                cashPaymentList.add(new CashPayment(cashPaymentId));
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void addCashPaymentListView(){
        cashpaymentcol.setCellValueFactory(new PropertyValueFactory<>("receiptNumber"));
        cashpaymenttable.setItems(cashPaymentList);
    }

    public ObservableList<FullBenefit> addFullBenefit(){
        ObservableList<FullBenefit> fullBenefitLists = FXCollections.observableArrayList();
        String sqlQuery = "SELECT * FROM FULL_BENEFITS";
        try {
            connection = DatabaseController.connection();
            preparedStatement = connection.prepareStatement(sqlQuery);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                ...
                String benefitName = resultSet.getString("half_health_coverage");
                fullBenefitLists.add(new FullBenefit(benefitId, benefitName));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return fullBenefitLists;
    }

    public void addFullBenefitListView(){
        fullbenefitidcol.setCellValueFactory(new PropertyValueFactory<>("benefit_id"));
        fullbenefitcol.setCellValueFactory(new PropertyValueFactory<>("fullBenefit"));
        fullbenefittable.setItems(fullBenefitList);
    }

    public ObservableList<HalfBenefit> addHalfBenefit(){
        ObservableList<HalfBenefit>  halfbenefitList= FXCollections.observableArrayList();
        String sqlQuery = "SELECT * FROM HALF_BENEFITS";
        try {
            connection = DatabaseController.connection();
            preparedStatement = connection.prepareStatement(sqlQuery);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                int benefitId = resultSet.getInt("benefit_id");
                String benefitName = resultSet.getString("half_health_coverage");
                halfbenefitList.add(new HalfBenefit(benefitId, benefitName));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return halfbenefitList;
    }

    public void addHalfBenefitListView(){
        halfbenefitidcol.setCellValueFactory(new PropertyValueFactory<>("benefit_id"));
        halfbenefitcol.setCellValueFactory(new PropertyValueFactory<>("halfBenefit"));
        halfbenefittable.setItems(halfbenefitList);
    }

    public ObservableList<NoBenefit> addNoBenefit(){
        ObservableList<NoBenefit>  noBenefitlist= FXCollections.observableArrayList();
        String sqlQuery = "SELECT * FROM NO_BENEFITS";
        try {
            connection = DatabaseController.connection();
            preparedStatement = connection.prepareStatement(sqlQuery);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                int benefitId = resultSet.getInt("benefit_id");
                String benefitName = resultSet.getString("no_health_coverage");
                noBenefitlist.add(new NoBenefit(benefitId, benefitName));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return noBenefitlist;
    }

    public void addNoBenefitListView(){
        nobenefitidcol.setCellValueFactory(new PropertyValueFactory<>("benefit_id"));
        nobenefitcol.setCellValueFactory(new PropertyValueFactory<>("noBenefit"));
        nobenefittable.setItems(noBenefitsList);
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        addEmpListView();
        addCashPaymentListView();
        addLocationListView();
        addHospitalListView();
        addCabinListView();
        addBillsListView();
        addFullBenefitListView();
        addHalfBenefitListView();
        addNoBenefitListView();
